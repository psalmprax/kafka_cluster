...
```
 ...
 ```mermaid
 flowchart TD
  A[Start InitContainer] --> B{Mount K8s Secrets (Raw Certs and Passwords)}
  ...
  H --> I{Write Password Files (e.g., kafka.server.keystore.password)}
  I --> J[Set File Permissions (chmod 600)]
  J --> K[Unset Password Env Vars]
  K --> L[End InitContainer]  %% "End" not "end"
 ```
